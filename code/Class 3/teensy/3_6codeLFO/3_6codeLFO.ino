...
void loop() {
  ...
  float lfoFrequency = 0.1;  // Frequency in Hertz
  float lfoAmplitude = 1.0;  // Amplitude of the LFO

  // Get current time (in milliseconds)
  unsigned long currentTime = millis();

  // Generate a sine wave LFO between -1 and 1
  float lfoValue = generateSineLFO(lfoFrequency, lfoAmplitude, currentTime);

  float lfoValueForAttack = map(lfoValue, -1, 1, 0, 7500);

  //read the pot positions and convert to a floating point b/w 0.0 and 1.0
  // attackInMilliseconds = map(analogRead(A17), 0, 1023, 0, 7500);
  attackInMilliseconds = lfoValueForAttack;
...
// Function to generate a sine wave LFO
float generateSineLFO(float frequency, float amplitude, unsigned long time) {
  // Calculate the angular frequency
  float omega = 2.0 * PI * frequency;

  // Calculate the sine value at the given time
  float sineValue = amplitude * sin(omega * time / 1000.0);  // Dividing by 1000 to convert milliseconds to seconds
